...
<script>
    const settings = new Settings({
        @if (Auth::guard('razen_holiday')->check())
            attributes: {
                color: "{{Auth::guard('razen_holiday')->user()->color_layout?Auth::guard('razen_holiday')->user()->color_layout:'light-blue'}}",
                navcolor: "{{Auth::guard('razen_holiday')->user()->nav_color?Auth::guard('razen_holiday')->user()->nav_color:'light'}}",
                behaviour: "{{Auth::guard('razen_holiday')->user()->behaviour?Auth::guard('razen_holiday')->user()->behaviour:'pinned'}}",
                layout: "{{Auth::guard('razen_holiday')->user()->layout?Auth::guard('razen_holiday')->user()->layout:'fluid'}}",
                radius: "{{Auth::guard('razen_holiday')->user()->radius?Auth::guard('razen_holiday')->user()->radius:'flat'}}",
                placement: "{{Auth::guard('razen_holiday')->user()->placement?Auth::guard('razen_holiday')->user()->placement:'vertical'}}",
            }
        @endif
    });
    $('.option').click(function(){
        ...
        $.ajax({
            url: "{{ route('razen-holiday.admin.dashboard.change') }}",
            method: 'POST',
            data: {
                "_token": "{{ csrf_token() }}",
