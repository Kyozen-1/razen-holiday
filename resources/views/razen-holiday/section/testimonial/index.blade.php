...
@section('title', 'Razen Holiday | Testimonial')
...
@section('content')
        ...
        <div class="col-12 col-md-7">
            ...
            <nav class="breadcrumb-container d-inline-block" aria-label="breadcrumb">
                <ul class="breadcrumb pt-0">
                    ...
                    <li class="breadcrumb-item"><a href="{{ route('razen-holiday.section.testimonial.index') }}">Testimonial</a></li>
                </ul>
            </nav>
        </div>
        <!-- Title End -->
    ...
    <div class="card mb-5">
        <div class="card-body">
            <h2 class="small-title">Edit Section Testimoni</h2>
            <div class="row mb-3">
                <div class="col-12">
                    <img src="{{ asset('images/section/testimonial/section_1.png') }}" alt="" class="img-fluid rounded">
                </div>
            </div>

            {{-- Form Start --}}
            <form action="{{ route('razen-holiday.section.testimonial.section.store') }}" novalidate="novalidate" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="row">
                    <div class="col-12 col-md-7">
                        <div class="mb-3 position-relative form-group">
                            <label for="" class="form-label">Sub Judul</label>
                            <input type="text" class="form-control" name="sub_judul" value="{{$section_testimonial?$section_testimonial->sub_judul : ''}}" required>
                        </div>
                        <div class="mb-3 position-relative form-group">
                            <label for="" class="form-label">Judul</label>
                            <input type="text" class="form-control" name="judul" value="{{$section_testimonial?$section_testimonial->judul : ''}}" required>
                        </div>
                        <div class="mb-3 position-relative form-group">
                            <label for="" class="form-label">Deskripsi</label>
                            <textarea name="deskripsi" rows="5" class="form-control" id="section_1" required>{{$section_testimonial?$section_testimonial->deskripsi : ''}}</textarea>
                        </div>
                    </div>
                    <div class="col-12 col-md-5">
                        <div class="position-relative form-group">
                            <label for="" class="form-label">Gambar Background</label>
                            @if ($section_testimonial)
                            <input type="file" class="dropify" name="gambar_background" data-height="300" data-allowed-file-extensions="png jpg jpeg" data-default-file="{{ asset('images/section/testimonial/'.$section_testimonial->gambar_background) }}" data-show-errors="true" required>
                            @else
                        ...
                        <div class="position-relative form-group">
                            ...
                            @if ($section_testimonial)
                            <input type="file" class="dropify" name="gambar" data-height="300" data-allowed-file-extensions="png jpg jpeg" data-default-file="{{ asset('images/section/testimonial/'.$section_testimonial->gambar) }}" data-show-errors="true" required>
                            @else
    ...
    <script>
        $(document).ready(function(){
            ...
            var dataTables = $('#testimoni_table').DataTable({
                ...
                serverSide: true,
                ajax: {
                    url: "{{ route('razen-holiday.section.testimonial.index') }}"
                },
                columns: [
                    {
                        data: 'DT_RowIndex',
        ...
        $('#testimoni_form').on('submit', function(e){
            ...
            {
                $.ajax({
                    url: "{{ route('razen-holiday.section.testimonial.store') }}",
                    method: "POST",
            ...
            {
                $.ajax({
                    url: "{{ route('razen-holiday.section.testimonial.update') }}",
                    method: "POST",
                    data: new FormData(this),
                    dataType: "json",
                    contentType: false,
        ...
        $(document.body).on('click', '.detail', function(){
            var id = $(this).attr('id');
            var url = "{{ route('razen-holiday.section.testimonial.show', ['id' => ":id"]) }}";
            ...
            $.ajax({
                ...
                success: function(data)
                {
                    ...
                    $('#detail_foto').attr('src', "{{ asset('images/testimoni') }}" + '/' + data.result.foto);
                    $('#detail').modal('show');
                }
            });
        });
        ...
        $(document).on('click', '.edit', function(){
            var id = $(this).attr('id');
            $('#form_result').html('');
            var url = "{{ route('razen-holiday.section.testimonial.edit', ['id' => ":id"]) }}";
            url = url.replace(':id', id);
            $.ajax({
                url: url,
                dataType: "json",
                success: function(data)
                {
                    ...
                    var lokasi_foto = "{{ asset('images/testimoni') }}"+'/'+data.result.foto;
        ...
        $('#ok_button').click(function(){
            var url = "{{ route('razen-holiday.section.testimonial.destroy', ['id' => ":id"]) }}";
            url = url.replace(":id", id);
            $.ajax({
                url: url,
                beforeSend: function(){
